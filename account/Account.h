...
class Balance {
public:
	// member function that sets the account balance in the object
	void setName(std::string balanceName) {
		name = balanceName; // store the account name           
	}

	// member function that retrieves the account balance from the object       
	std::string getName() const {
		return name; // return name's value to this function's caller
	}
private:
	std::string name; // data member containing account holder's balance
}; // end class Account
